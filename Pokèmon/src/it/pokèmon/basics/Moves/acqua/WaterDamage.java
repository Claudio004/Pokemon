...
public class WaterDamage {
	...
	WaterDamage(Pokèmon p, Pokèmon p2, Vulnerability vul, int movePower){
		this.p = p;
		this.p2 = p2;
		this.vul = vul;
	...
	public void damage() {
		Boolean Acquaiuto = false;
		if(p.getAbility() == Abilities.ACQUAIUTO || p.getH_ability() == HiddenAbilities.ACQUAIUTO) {
			Acquaiuto = true;
		}
		if(p2.getAbility() == Abilities.ASSORBACQUA || p.getH_ability() == HiddenAbilities.ASSORBACQUA) {
			...
			if(vul.damageMultiplier() == 0.5) {
				if(p.getHP() <= (p.getMaxHP() / 3) && Acquaiuto) {
