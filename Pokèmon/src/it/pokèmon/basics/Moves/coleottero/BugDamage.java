 ...
 * @since 2.0
 */
public class BugDamage {
	Vulnerability vul;
	Pokèmon p;
	Pokèmon p2;
	int movePower;
	
	BugDamage(Pokèmon p, Pokèmon p2, Vulnerability vul, int movePower){
		this.p = p;
		this.p2 = p2;
		this.vul = vul;
	...
	public void damage() {
		Boolean Aiutinsetto = false;
		if(p.getAbility() == Abilities.AIUTINSETTO || p.getH_ability() == HiddenAbilities.AIUTINSETTO) {
			Aiutinsetto = true;
		}
		if(vul.damageMultiplier() == 0.5) {
			if(p.getHP() <= (p.getMaxHP() / 3) && Aiutinsetto) {
